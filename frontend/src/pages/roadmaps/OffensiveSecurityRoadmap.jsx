// frontend/src/pages/roadmaps/OffensiveSecurityRoadmap.jsx
import React, { useEffect, useState } from 'react';
import { Link, useNavigate, useSearchParams } from 'react-router-dom';
import { useAuth } from '../../hooks/useAuth.jsx';
import { useTheme } from '../../contexts/ThemeContext.jsx';

// Offensive Security & Red Teaming Roadmap Data
  ...
  return (
    <div className={`mt-8 p-6 md:p-8 rounded-xl border-2 text-center ${
      ...
      <p className={`text-lg md:text-xl mb-6 ${isDark ? 'text-green-200' : 'text-green-800'}`}>
        You've completed the Offensive Security & Red Teaming Roadmap!
      </p>
      <div className={`p-4 rounded-lg ${isDark ? 'bg-green-800/30' : 'bg-green-100'}`}>
        <p className={`font-semibold ${isDark ? 'text-green-300' : 'text-green-700'}`}>
          You can now perform professional penetration testing, red team operations, advanced exploitation, and are ready for real-world offensive security challenges!
        </p>
      </div>
    </div>
  );
    ...
    <div className="mt-12 space-y-8">
      ...
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'border-primary-500/50 bg-primary-900/30' : 'border-primary-300 bg-primary-50/80'}`}>
        ...
        <p className={`text-lg leading-relaxed ${isDark ? 'text-primary-100' : 'text-primary-700'}`}>
          {OFFENSIVE_SECURITY_ROADMAP.outcome}
        </p>
      </div>
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {OFFENSIVE_SECURITY_ROADMAP.career_paths.map((path, index) => (
            <div 
              key={index}
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {OFFENSIVE_SECURITY_ROADMAP.mini_projects.map((project, index) => (
            <div 
              key={index}
              className={`p-4 rounded-lg border-2 transition-all duration-300 hover:scale-105 ${
                isDark ? 'border-gray-600 bg-gray-800 hover:bg-gray-750' : 'border-gray-200 bg-white hover:bg-gray-50'
      ...
      <div>
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          {OFFENSIVE_SECURITY_ROADMAP.tools.map((tool, index) => (
            <div 
              key={index}
              className={`p-3 rounded-lg border transition-all duration-300 ${
                isDark ? 'border-red-500/30 bg-red-900/20 hover:bg-red-900/30' : 'border-red-200 bg-red-50 hover:bg-red-100'
              }`}
            >
              <div className="flex items-center">
                <span className="w-2 h-2 bg-red-500 rounded-full mr-2 flex-shrink-0"></span>
                <span className={`text-sm ${isDark ? 'text-red-300' : 'text-red-700'}`}>{tool}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'border-red-500/50 bg-red-900/20' : 'border-red-300 bg-red-50/80'}`}>
        <h3 className={`text-xl font-semibold mb-4 ${isDark ? 'text-red-300' : 'text-red-800'}`}>
          🎯 Practice Labs & Platforms
        </h3>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-red-200' : 'text-red-700'}`}>Learning Platforms</h4>
            <ul className={`space-y-1 text-sm ${isDark ? 'text-red-300' : 'text-red-600'}`}>
              {OFFENSIVE_SECURITY_ROADMAP.labs.slice(0, 3).map((lab, index) => (
                <li key={index}>• {lab}</li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-red-200' : 'text-red-700'}`}>Vulnerable Environments</h4>
            <ul className={`space-y-1 text-sm ${isDark ? 'text-red-300' : 'text-red-600'}`}>
              {OFFENSIVE_SECURITY_ROADMAP.labs.slice(3).map((lab, index) => (
                <li key={index}>• {lab}</li>
              ))}
            </ul>
          </div>
      ...
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'border-orange-500/50 bg-orange-900/20' : 'border-orange-300 bg-orange-50/80'}`}>
        <h3 className={`text-xl font-semibold mb-4 ${isDark ? 'text-orange-300' : 'text-orange-800'}`}>
          🛡️ Attack Methodologies You'll Master
        </h3>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-orange-200' : 'text-orange-700'}`}>MITRE ATT&CK Framework</h4>
            <p className={`text-sm ${isDark ? 'text-orange-300' : 'text-orange-600'}`}>
              Master reconnaissance, resource development, initial access, execution, persistence, privilege escalation, defense evasion, credential access, discovery, lateral movement, collection, command and control, exfiltration, and impact.
            </p>
          </div>
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-orange-200' : 'text-orange-700'}`}>Cyber Kill Chain</h4>
            <p className={`text-sm ${isDark ? 'text-orange-300' : 'text-orange-600'}`}>
              Learn reconnaissance, weaponization, delivery, exploitation, installation, command and control, and actions on objectives for comprehensive attack simulation.
            </p>
          </div>
        </div>
      </div>
...
// Main Offensive Security Roadmap Component
const OffensiveSecurityRoadmap = () => {
  ...
  useEffect(() => {
    ...
    const loadProgress = () => {
      try {
        const savedProgress = localStorage.getItem('offensive-security-roadmap-progress');
  ...
  const handleMarkComplete = (phaseIndex) => {
    ...
    if (!newCompletedPhases.includes(phaseIndex)) {
      ...
      localStorage.setItem('offensive-security-roadmap-progress', JSON.stringify(newCompletedPhases));
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem('offensive-security-roadmap-progress', JSON.stringify(completedPhases));
  ...
  const isPhaseActive = (phaseIndex) => {
    ...
    const firstIncomplete = OFFENSIVE_SECURITY_ROADMAP.phases.findIndex((_, index) => !isPhaseCompleted(index));
  ...
  const totalPhases = OFFENSIVE_SECURITY_ROADMAP.phases.length;
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'dark-gradient-secondary' : 'bg-gray-50'}`}>
        <div className="text-center">
          ...
          <p className={`mt-4 ${isDark ? 'text-white' : 'text-gray-700'}`}>Loading your Red Team progress...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="gradient-bg text-white py-8 md:py-12 lg:py-16 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
          <div className="flex flex-col lg:flex-row items-start justify-between gap-6 lg:gap-8">
            <div className="flex-1">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">{OFFENSIVE_SECURITY_ROADMAP.title}</h1>
              <p className="text-base md:text-lg lg:text-xl text-gray-300 mb-6 max-w-3xl">
                {OFFENSIVE_SECURITY_ROADMAP.description}
              </p>
              
              <div className="flex flex-wrap gap-3 md:gap-4 lg:gap-6 text-xs md:text-sm">
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-primary-400 rounded-full mr-2"></span>
                  <span>{Math.ceil(OFFENSIVE_SECURITY_ROADMAP.estimated_hours / 20)} weeks • {OFFENSIVE_SECURITY_ROADMAP.estimated_hours} hours</span>
                </div>
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-green-400 rounded-full mr-2"></span>
                  <span>{OFFENSIVE_SECURITY_ROADMAP.difficulty}</span>
                </div>
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-red-400 rounded-full mr-2"></span>
                  <span>{OFFENSIVE_SECURITY_ROADMAP.prerequisites.length} prerequisites</span>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 lg:py-12">
        <div id="learning-path">
          ...
          <div className="mb-8">
            ...
            >
              {OFFENSIVE_SECURITY_ROADMAP.phases.map((phase, index) => (
                <div key={index}>
                  <PhaseHeader
                    phase={phase}
                    ...
                    isDark={isDark}
                    index={index}
                    isLast={index === OFFENSIVE_SECURITY_ROADMAP.phases.length - 1}
