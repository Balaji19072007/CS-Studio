// frontend/src/pages/roadmaps/ObservabilityReliabilityRoadmap.jsx
import React, { useEffect, useState } from 'react';
import { Link, useNavigate, useSearchParams } from 'react-router-dom';
import { useAuth } from '../../hooks/useAuth.jsx';
import { useTheme } from '../../contexts/ThemeContext.jsx';

// Observability & Reliability Roadmap Data
  ...
  return (
    <div className={`mt-8 p-6 md:p-8 rounded-xl border-2 text-center ${
      ...
      <p className={`text-lg md:text-xl mb-6 ${isDark ? 'text-green-200' : 'text-green-800'}`}>
        You've completed the Observability & Reliability Roadmap!
      </p>
      <div className={`p-4 rounded-lg ${isDark ? 'bg-green-800/30' : 'bg-green-100'}`}>
        <p className={`font-semibold ${isDark ? 'text-green-300' : 'text-green-700'}`}>
          You are now equipped to build world-class observability setups and ensure production system reliability!
        </p>
      </div>
    </div>
  );
    ...
    <div className="mt-12 space-y-8">
      ...
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'border-primary-500/50 bg-primary-900/30' : 'border-primary-300 bg-primary-50/80'}`}>
        ...
        <p className={`text-lg leading-relaxed ${isDark ? 'text-primary-100' : 'text-primary-700'}`}>
          {OBSERVABILITY_RELIABILITY_ROADMAP.outcome}
        </p>
      </div>
      ...
      <div>
        <h3 className={`text-xl font-semibold mb-6 ${isDark ? 'text-white' : 'text-gray-900'}`}>
          🚀 Career Paths in Observability & Reliability
        </h3>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {OBSERVABILITY_RELIABILITY_ROADMAP.career_paths.map((path, index) => (
            <div 
              key={index}
      ...
      <div>
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          {OBSERVABILITY_RELIABILITY_ROADMAP.tools.map((tool, index) => (
            <div 
              key={index}
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {OBSERVABILITY_RELIABILITY_ROADMAP.mini_projects.map((project, index) => (
            <div 
              key={index}
              className={`p-4 rounded-lg border-2 transition-all duration-300 hover:scale-105 ${
                isDark ? 'border-gray-600 bg-gray-800 hover:bg-gray-750' : 'border-gray-200 bg-white hover:bg-gray-50'
...
// Main Observability & Reliability Roadmap Component
const ObservabilityReliabilityRoadmap = () => {
  ...
  useEffect(() => {
    ...
    const loadProgress = () => {
      try {
        const savedProgress = localStorage.getItem('observability-reliability-roadmap-progress');
  ...
  const handleMarkComplete = (phaseIndex) => {
    ...
    if (!newCompletedPhases.includes(phaseIndex)) {
      ...
      localStorage.setItem('observability-reliability-roadmap-progress', JSON.stringify(newCompletedPhases));
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem('observability-reliability-roadmap-progress', JSON.stringify(completedPhases));
  ...
  const isPhaseActive = (phaseIndex) => {
    ...
    const firstIncomplete = OBSERVABILITY_RELIABILITY_ROADMAP.phases.findIndex((_, index) => !isPhaseCompleted(index));
  ...
  const totalPhases = OBSERVABILITY_RELIABILITY_ROADMAP.phases.length;
      ...
      <div className="gradient-bg text-white py-8 md:py-12 lg:py-16 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
          <div className="flex flex-col lg:flex-row items-start justify-between gap-6 lg:gap-8">
            <div className="flex-1">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">{OBSERVABILITY_RELIABILITY_ROADMAP.title}</h1>
              <p className="text-base md:text-lg lg:text-xl text-gray-300 mb-6 max-w-3xl">
                {OBSERVABILITY_RELIABILITY_ROADMAP.description}
              </p>
              
              <div className="flex flex-wrap gap-3 md:gap-4 lg:gap-6 text-xs md:text-sm">
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-primary-400 rounded-full mr-2"></span>
                  <span>{Math.ceil(OBSERVABILITY_RELIABILITY_ROADMAP.estimated_hours / 20)} weeks • {OBSERVABILITY_RELIABILITY_ROADMAP.estimated_hours} hours</span>
                </div>
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-green-400 rounded-full mr-2"></span>
                  <span>{OBSERVABILITY_RELIABILITY_ROADMAP.difficulty}</span>
                </div>
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-blue-400 rounded-full mr-2"></span>
                  <span>{OBSERVABILITY_RELIABILITY_ROADMAP.prerequisites.length} prerequisites</span>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 lg:py-12">
        <div id="learning-path">
          ...
          <div className="mb-8">
            ...
            >
              {OBSERVABILITY_RELIABILITY_ROADMAP.phases.map((phase, index) => (
                <div key={index}>
                  <PhaseHeader
                    phase={phase}
                    ...
                    isDark={isDark}
                    index={index}
                    isLast={index === OBSERVABILITY_RELIABILITY_ROADMAP.phases.length - 1}
