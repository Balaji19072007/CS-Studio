// frontend/src/pages/roadmaps/DevOpsCICDRoadmap.jsx
import React, { useEffect, useState } from 'react';
import { Link, useNavigate, useSearchParams } from 'react-router-dom';
import { useAuth } from '../../hooks/useAuth.jsx';
import { useTheme } from '../../contexts/ThemeContext.jsx';

// DevOps Fundamentals & CI/CD Roadmap Data
  ...
  return (
    <div className={`mt-8 p-6 md:p-8 rounded-xl border-2 text-center ${
      ...
      <p className={`text-lg md:text-xl mb-6 ${isDark ? 'text-green-200' : 'text-green-800'}`}>
        You've completed the DevOps Fundamentals & CI/CD Roadmap!
      </p>
      <div className={`p-4 rounded-lg ${isDark ? 'bg-green-800/30' : 'bg-green-100'}`}>
        <p className={`font-semibold ${isDark ? 'text-green-300' : 'text-green-700'}`}>
          You can now implement DevOps practices, use Git professionally, build CI/CD pipelines, automate workflows, and deploy applications automatically!
        </p>
      </div>
    </div>
  );
    ...
    <div className="mt-12 space-y-8">
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {DEVOPS_CICD_ROADMAP.career_paths.map((path, index) => (
            <div 
              key={index}
            ...
            >
              <div className="flex items-center">
                <span className="w-3 h-3 bg-cyan-500 rounded-full mr-3 flex-shrink-0"></span>
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {DEVOPS_CICD_ROADMAP.mini_projects.map((project, index) => (
            <div 
              key={index}
              className={`p-4 rounded-lg border-2 transition-all duration-300 hover:scale-105 ${
                isDark ? 'border-gray-600 bg-gray-800 hover:bg-gray-750' : 'border-gray-200 bg-white hover:bg-gray-50'
      ...
      <div>
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          {DEVOPS_CICD_ROADMAP.tools.map((tool, index) => (
            <div 
              key={index}
              className={`p-3 rounded-lg border transition-all duration-300 ${
                isDark ? 'border-cyan-500/30 bg-cyan-900/20 hover:bg-cyan-900/30' : 'border-cyan-200 bg-cyan-50 hover:bg-cyan-100'
              }`}
            >
              <div className="flex items-center">
                <span className="w-2 h-2 bg-cyan-500 rounded-full mr-2 flex-shrink-0"></span>
                <span className={`text-sm ${isDark ? 'text-cyan-300' : 'text-cyan-700'}`}>{tool}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Platforms Section */}
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'border-cyan-500/50 bg-cyan-900/20' : 'border-cyan-300 bg-cyan-50/80'}`}>
        <h3 className={`text-xl font-semibold mb-4 ${isDark ? 'text-cyan-300' : 'text-cyan-800'}`}>
          🚀 Deployment Platforms
        </h3>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-cyan-200' : 'text-cyan-700'}`}>Git Platforms</h4>
            <ul className={`space-y-1 text-sm ${isDark ? 'text-cyan-300' : 'text-cyan-600'}`}>
              {DEVOPS_CICD_ROADMAP.platforms.slice(0, 2).map((platform, index) => (
                <li key={index}>• {platform}</li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-cyan-200' : 'text-cyan-700'}`}>Deployment Services</h4>
            <ul className={`space-y-1 text-sm ${isDark ? 'text-cyan-300' : 'text-cyan-600'}`}>
              {DEVOPS_CICD_ROADMAP.platforms.slice(2).map((platform, index) => (
                <li key={index}>• {platform}</li>
              ))}
            </ul>
          </div>
      ...
      {/* DevOps Principles */}
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'border-blue-500/50 bg-blue-900/20' : 'border-blue-300 bg-blue-50/80'}`}>
        <h3 className={`text-xl font-semibold mb-4 ${isDark ? 'text-blue-300' : 'text-blue-800'}`}>
          🔄 DevOps Principles You'll Master
        </h3>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-blue-200' : 'text-blue-700'}`}>Culture & Collaboration</h4>
            <p className={`text-sm ${isDark ? 'text-blue-300' : 'text-blue-600'}`}>
              Breaking down silos between development and operations teams, fostering shared responsibility and continuous feedback.
            </p>
          </div>
          <div>
            <h4 className={`font-semibold mb-2 ${isDark ? 'text-blue-200' : 'text-blue-700'}`}>Automation & Efficiency</h4>
            <p className={`text-sm ${isDark ? 'text-blue-300' : 'text-blue-600'}`}>
              Automating repetitive tasks, implementing CI/CD pipelines, and optimizing workflows for faster, more reliable software delivery.
            </p>
          </div>
        </div>
      </div>
...
// Main DevOps & CI/CD Roadmap Component
const DevOpsCICDRoadmap = () => {
  ...
  useEffect(() => {
    ...
    const loadProgress = () => {
      try {
        const savedProgress = localStorage.getItem('devops-cicd-roadmap-progress');
  ...
  const handleMarkComplete = (phaseIndex) => {
    ...
    if (!newCompletedPhases.includes(phaseIndex)) {
      ...
      localStorage.setItem('devops-cicd-roadmap-progress', JSON.stringify(newCompletedPhases));
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem('devops-cicd-roadmap-progress', JSON.stringify(completedPhases));
  ...
  const isPhaseActive = (phaseIndex) => {
    ...
    const firstIncomplete = DEVOPS_CICD_ROADMAP.phases.findIndex((_, index) => !isPhaseCompleted(index));
  ...
  const totalPhases = DEVOPS_CICD_ROADMAP.phases.length;
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'dark-gradient-secondary' : 'bg-gray-50'}`}>
        <div className="text-center">
          ...
          <p className={`mt-4 ${isDark ? 'text-white' : 'text-gray-700'}`}>Loading your DevOps progress...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="gradient-bg text-white py-8 md:py-12 lg:py-16 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
          <div className="flex flex-col lg:flex-row items-start justify-between gap-6 lg:gap-8">
            <div className="flex-1">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">{DEVOPS_CICD_ROADMAP.title}</h1>
              <p className="text-base md:text-lg lg:text-xl text-gray-300 mb-6 max-w-3xl">
                {DEVOPS_CICD_ROADMAP.description}
              </p>
              
              <div className="flex flex-wrap gap-3 md:gap-4 lg:gap-6 text-xs md:text-sm">
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-primary-400 rounded-full mr-2"></span>
                  <span>{Math.ceil(DEVOPS_CICD_ROADMAP.estimated_hours / 20)} weeks • {DEVOPS_CICD_ROADMAP.estimated_hours} hours</span>
                </div>
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-green-400 rounded-full mr-2"></span>
                  <span>{DEVOPS_CICD_ROADMAP.difficulty}</span>
                </div>
                <div className="flex items-center text-gray-300">
                  <span className="w-2 h-2 bg-cyan-400 rounded-full mr-2"></span>
                  <span>{DEVOPS_CICD_ROADMAP.prerequisites.length} prerequisites</span>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 lg:py-12">
        <div id="learning-path">
          ...
          <div className="mb-8">
            ...
            >
              {DEVOPS_CICD_ROADMAP.phases.map((phase, index) => (
                <div key={index}>
                  <PhaseHeader
                    phase={phase}
                    ...
                    isDark={isDark}
                    index={index}
                    isLast={index === DEVOPS_CICD_ROADMAP.phases.length - 1}
