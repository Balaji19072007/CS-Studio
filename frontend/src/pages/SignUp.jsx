...
// Changed: Use hook instead of component for custom styling
...
const SignUp = () => {
  ...
  // --- Google Sign Up Handler (Updated to use hook and match SignIn) ---
  const handleGoogleSuccess = async (tokenResponse) => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.GOOGLE_AUTH.CALLBACK, {
        ...
        body: JSON.stringify({
          // Send the access token. Backend controller now handles both idToken and access_token key, 
          // but we'll use access_token to be explicit about what we are sending from this hook.
      ...
      console.error('Google Sign Up network error:', error);
      showMessage('error', 'Network error. Please try again.');
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleError = () => {
    showMessage('error', 'Google Sign Up was unsuccessful. Please try again.');
  };
